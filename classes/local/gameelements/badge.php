...
class badge extends game_element {
    ...
    public static function update_course_parameters(int $courseid, int $badgegold, int $badgesilver, $badgebronze): bool {
        ...
        $gameelements = $DB->get_records('ludimoodle_gameelements', ['courseid' => $courseid, 'type' => 'badge']);
        ...
        if (!$gameelements) {
            ...
            foreach ($gameelements as $gameelement) {
                ...
                $badgegoldrecord = $DB->get_record('ludimoodle_params',
                ...
                if ($badgegoldrecord) {
                    ...
                    $badgegoldrecord->value = $badgegold;
                    $DB->insert_record('ludimoodle_params', $badgegoldrecord);
                }

                // Retrieve existing values for badgesilver parameter.
                $badgesilverrecord = $DB->get_record('ludimoodle_params',
                    ['gameelementid' => $gameelement->id,
                        'name' => 'badgesilver']);
                // Check value.
                if ($badgesilver < 0)
                ...
                if ($badgesilverrecord) {
                    ...
                    $badgesilverrecord->value = $badgesilver;
                    $DB->insert_record('ludimoodle_params', $badgesilverrecord);
                }

                // Retrieve existing values for badgebronze parameter.
                $badgebronzerecord = $DB->get_record('ludimoodle_params',
                    ['gameelementid' => $gameelement->id,
                        'name' => 'badgebronze']);
                // Check value.
                if ($badgebronze < 0)
                ...
                if ($badgebronzerecord) {
                    ...
                    $badgebronzerecord->value = $badgebronze;
                    $DB->insert_record('ludimoodle_params', $badgebronzerecord);
                }
            }
        }
        return true;
