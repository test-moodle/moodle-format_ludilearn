...
class progress extends game_element {
    ...
    public static function update_elements(int $courseid, stdClass $coursemodule, string $modulename, int $userid): void {
        ...
        // Get game element.
        $gameelement = $DB->get_record('format_ludimoodle_gameelements',
            ['sectionid' => $coursemodule->section, 'type' => 'progress']);

        // Verify attribution.
        $attribution = $DB->get_record('format_ludimoodle_attribution',
    ...
    public static function get(int $courseid, int $sectionid, int $userid): ?progress {
        ...
        $gameelementsql = 'SELECT * FROM {format_ludimoodle_gameelements} g
                            INNER JOIN {format_ludimoodle_attribution} a ON g.id = a.gameelementid
        ...
        $sqlgameeleuser = 'SELECT s.id, s.name, s.value
                    FROM {format_ludimoodle_gameele_user} s
                    INNER JOIN {format_ludimoodle_attribution} a ON s.attributionid = a.id
        ...
        $sqlcms = 'SELECT cm.id, cm.cmid, cm.name, cm.value
                    ...
                    INNER JOIN {format_ludimoodle_attribution} a ON cm.attributionid = a.id
                    WHERE a.gameelementid = :gameelementid
                    AND a.userid = :userid';
